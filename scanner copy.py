...
def scan(
    ...
    # ── Fetch company names + sectors (one profile call covers both) ─────────────
    log.info("Fetching company names and sectors…")
    ...
    try:
        ...
        sectored= sum(1 for v in sector_map.values() if v)
        log.info("Company names: %d/%d  Sectors: %d/%d",
                 named, len(tickers), sectored, len(tickers))
    except Exception as e:
        log.debug("Company name/sector fetch failed: %s", e)
    ...
    if results and config.GREEKS_ENABLED:
        ...
        from signals.greeks import analyze_greeks
        for ta in results:
            direction = "buy" if ta.net_score > 0 else "sell"
            try:
                ta.gamma_data = analyze_greeks(
                    ticker    = ta.ticker,
                    price     = ta.price,
                    net_score = ta.net_score,
                    direction = direction,
                ...
                log.debug("Greeks failed %s: %s", ta.ticker, e)
        filled = sum(1 for ta in results if ta.gamma_data and ta.gamma_data.nearest_strike)
        log.info("Greeks complete: %d/%d tickers with option data", filled, len(results))
